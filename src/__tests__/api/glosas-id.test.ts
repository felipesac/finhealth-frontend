...
vi.mock('@/lib/rate-limit', () => ({
  ...
  getRateLimitKey: vi.fn().mockReturnValue('glosas-detail:127.0.0.1'),
}));
...
import { GET } from '@/app/api/glosas/[id]/route';
...
function makeGetReq() {
  return new Request(`http://localhost:3000/api/glosas/${testId}`, { method: 'GET' });
}

describe('GET /api/glosas/[id]', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    (mockChain as Record<string, unknown>).then = vi.fn((resolve: (v: unknown) => void) => resolve({
      data: { id: testId, glosa_type: 'technical', amount: 500, status: 'open' },
      error: null,
    }));
  });
  ...
  it('returns 200 with glosa data', async () => {
    const res = await GET(makeGetReq(), routeParams);
  ...
  it('returns 404 when glosa not found', async () => {
    (mockChain as Record<string, unknown>).then = vi.fn((resolve: (v: unknown) => void) => resolve({
      data: null,
