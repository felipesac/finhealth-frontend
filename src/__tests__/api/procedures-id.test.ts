...
vi.mock('@/lib/rate-limit', () => ({
  ...
  getRateLimitKey: vi.fn().mockReturnValue('procedures-detail:127.0.0.1'),
}));
...
import { GET } from '@/app/api/procedures/[id]/route';
...
function makeGetReq() {
  return new Request(`http://localhost:3000/api/procedures/${testId}`, { method: 'GET' });
}
...
describe('GET /api/procedures/[id]', () => {
  beforeEach(() => {
    ...
    (mockChain as Record<string, unknown>).then = vi.fn((resolve: (v: unknown) => void) => resolve({
      data: { id: testId, code: 'PROC-001', description: 'Test Procedure', quantity: 1 },
      error: null,
    }));
  });
  ...
  it('returns 200 with procedure data', async () => {
    const res = await GET(makeGetReq(), routeParams);
