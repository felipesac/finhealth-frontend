...
export async function GET(
  ...
  try {
    ...
    const rlKey = getRateLimitKey(request, 'glosas-detail');
    ...
    const { data, error } = await supabase
      .from('glosas')
      .select('*, medical_account:medical_accounts(account_number)')
      .eq('id', id)
    ...
    if (error || !data) {
      return NextResponse.json(
        { success: false, error: 'Glosa nao encontrada' },
        { status: 404 }
      );
    }
    ...
    return NextResponse.json(
      { success: false, error: err.message || 'Falha ao buscar glosa' },
      { status: 500 }
    );
  }
}
