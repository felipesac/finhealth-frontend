...
export async function GET(
  ...
  try {
    ...
    const rlKey = getRateLimitKey(request, 'accounts-detail');
    ...
    const { data, error } = await supabase
      ...
      .select('*, patient:patients(id, name), health_insurer:health_insurers(id, name)')
      .eq('id', id)
    ...
    return NextResponse.json(
      { success: false, error: err.message || 'Falha ao buscar conta' },
      { status: 500 }
    );
  }
}
